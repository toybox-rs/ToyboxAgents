#!/bin/bash 
#
#SBATCH --output=exp_StayAliveJitter_AimLeft.out
#SBATCH -e exp_StayAliveJitter_AimLeft.err
#SBATCH --time=0-11:59
...
if [ "$1" = "local" ]; then 
  ...
  python -m autoexp \
    ...
    --outdir exp/StayAliveJitter/AimLeft \
    --datadir $training_data \
    --constraints 'bricks\[.*?\].*' \
    1> exp_StayAliveJitter_AimLeft.out 2> exp_StayAliveJitter_AimLeft.err
elif [ "$1" = "swarm" ]; then
  ...
  python -m autoexp breakout \
    ...
    --outcome Aim left \
    --counterfactual Aim right \
    --outdir $WORK1/autoexp/exp/StayAliveJitter/AimLeft \
    --seed 6232020 \
    --maxsteps 2000 \
    --window 64 \
    --datadir $training_data
