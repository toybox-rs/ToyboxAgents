#!/bin/bash 
#
#SBATCH --output=exp_StayAlive_HitBall.out
#SBATCH -e exp_StayAlive_HitBall.err
#SBATCH --time=0-11:59
...
if [ "$1" = "local" ]; then 
  ...
  python -m autoexp \
    ...
    --outdir exp/StayAlive/HitBall \
    --datadir $training_data \
    --constraints 'bricks\[.*?\].*' \
    1> exp_StayAlive_HitBall.out 2> exp_StayAlive_HitBall.err
elif [ "$1" = "swarm" ]; then
  ...
  python -m autoexp breakout \
    ...
    --outcome HitBall \
    --counterfactual MissedBall \
    --outdir $WORK1/autoexp/exp/StayAlive/HitBall \
    --seed 6232020 \
    --maxsteps 2000 \
    --window 64 \
    --datadir $training_data
