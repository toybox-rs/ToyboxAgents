#!/bin/bash 
#
#SBATCH --output=exp_StayAlive_AimRight.out
#SBATCH -e exp_StayAlive_AimRight.err
#SBATCH --time=0-11:59
...
if [ "$1" = "local" ]; then 
  ...
  python -m autoexp \
    ...
    --outdir exp/StayAlive/AimRight \
    --datadir $training_data \
    --constraints 'bricks\[.*?\].*' \
    1> exp_StayAlive_AimRight.out 2> exp_StayAlive_AimRight.err
elif [ "$1" = "swarm" ]; then
  ...
  python -m autoexp breakout \
    ...
    --outcome Aim right \
    --counterfactual Aim left \
    --outdir $WORK1/autoexp/exp/StayAlive/AimRight \
    --seed 6232020 \
    --maxsteps 2000 \
    --window 64 \
    --datadir $training_data
